...
std::string EvaUtil::smileyToText(unsigned short smileyCode)
{
	printf("0x%2x, ", 0xffff&smileyCode);
	/*
	int i = smileyToFileIndex(smileyCode);
	std::string text = map[i].py;
	*/
	char szText[16];
	sprintf(szText,"[face:%d]",smileyCode);
	return szText;
}

unsigned char EvaUtil::textToSmiley(const std::string &textTag)
{
	std::string strEn, strPy;
	for(int i=0; i<QQ_SMILEY_AMOUNT; i++){
		strEn = map[i].en;
		if (strEn.length()==0) continue;

		strPy = map[i].py;
		if(strEn == textTag || strPy == textTag){
			return map[i].index06;
		}
	}
	return 0;
}
...
int EvaUtil::smileyToFileIndex( const unsigned char smileyCode)
{
	for(int i=0; i<QQ_SMILEY_AMOUNT; i++){
		if(map[i].index06==smileyCode)
			return i;
	}
        return -1;
}
...
void EvaUtil::initMap()
{
	...
	*/
	...
	if (FILE* fp=fopen("plugins\\qqemot.txt","r")) {
		...
		memset(map,0,sizeof(textMap)*256);

		while (fgets(szTemp,MAX_PATH,fp)) {
			if (*szTemp==';' || *szTemp==0 || *szTemp=='\r' || *szTemp=='\n') continue;

			sscanf(szTemp,"%x\t%x\t%s\t%s",&idx09,&idx06,szEn,szPy);
			pTM=map+idx09;
			pTM->index06=idx06;
			pTM->index09=idx09;
			strcpy(pTM->en,szEn);
...
std::string EvaUtil::customSmileyToText( const char * buf, int * smileyLength, const char *uuid)
{
	...
	case 0x36:{
		...
		partLen[3]=0x00;
		int len = atoi(partLen);util_log(0,"36 len:%d\n",len);
		delete partLen;
