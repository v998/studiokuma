...
int SendIM::putBody(unsigned char *buf) 
{
	...
	pos+=EvaUtil::write32(buf+pos,0x00000008);
	pos+=EvaUtil::write32(buf+pos,0x00010004);
	pos+=EvaUtil::write32(buf+pos,0x00000000);
...
int SendTextIMPacket::putContents(unsigned char *buf) 
{
	...
	/*
	bool hasImage = false;
	char* pszUTF8=mir_utf8encodecp(message.c_str(),936);
	std::string str2send = EvaUtil::convertToSend(pszUTF8, &hasImage);
	mir_free(pszUTF8);
	*/

	buf[pos++] = /*hasImage?QQ_IM_IMAGE_REPLY:*/replyType; // auto-reply or not
	pos+=EvaUtil::write32(buf+pos,0x4D534700 ); //"MSG"
	pos+=EvaUtil::write32(buf+pos,0x00000000 );
	pos+=EvaUtil::write32(buf+pos,time(NULL) );
	pos+=EvaUtil::write32(buf+pos,rand());
	// pos+=EvaUtil::write32(buf+pos,0x00000000);
	buf[pos++]=0;
	buf[pos++]=blue;
	buf[pos++]=green;
	...
	if (bold) buf[pos]|=1;
	if (italic) buf[pos]|=2;
	if (underline) buf[pos]|=4;
	pos++;
	pos+=EvaUtil::write16(buf+pos,0x8600);

	char* pszUTF8=mir_utf8encodecp(fontName.c_str(),936);
	unsigned short len=strlen(pszUTF8);
	pos+=EvaUtil::write16(buf+pos,len);
	memcpy(buf+pos,pszUTF8,len);
	mir_free(pszUTF8);
	pos+=len;
	pos+=EvaUtil::write16(buf+pos,0x0000);

	// TODO: Handles smileys
	/*
	buf[pos++]=0x01;
	len=str2send.length();
	unsigned short* pwLen=(unsigned short*)(buf+pos); // Write later
	pos+=2;
	unsigned short posFrom=pos;
	*/

	char* msg=mir_strdup(EvaUtil::convertToSend(message).c_str()); // This is message in GBK
	char* pMsg=msg;
	char* psz14;
	char* psz15;
	...
	while (*pMsg) {
		...
		if (psz14!=pMsg && psz15!=pMsg) {
			...
			pszUTF8=mir_utf8encodecp(pMsg,936);

			len=strlen(pszUTF8);
			buf[pos++]=0x01; // content-type
			pos+=EvaUtil::write16(buf+pos,len+3); // Size of this section
			buf[pos++]=0x01; // content indicator
			pos+=EvaUtil::write16(buf+pos,len); // Size of content
			memcpy(buf+pos,pszUTF8,len);
			pos+=len;
			pMsg+=strlen(pMsg);
		}
		...
		if (psz14) {
			// Write smiley
			buf[pos++]=0x02; // content-type
			pos+=EvaUtil::write16(buf+pos,9); // Size of this section
			buf[pos++]=0x01; // unknown
			pos+=EvaUtil::write16(buf+pos,1); // Length: 1 byte
			buf[pos++]=EvaUtil::smileyToFileIndex(psz14[1]); //(unsigned char)psz14[1]-0x41;
			buf[pos++]=0xff; // content indicator
			pos+=EvaUtil::write16(buf+pos,2); // Size of content
			buf[pos++]=0x14;
			buf[pos++]=psz14[1];
			pMsg=psz14+2;
		} else if (psz15) {
			// Write image
			char szLen[10];
			unsigned char cbShortcut;
			unsigned char cbFilename;
			...
			buf[pos++]=0x03; // content-type
			pos+=EvaUtil::write16(buf+pos,53+len+cbFilename);

			buf[pos++]=0x02; // Filename
			memcpy(buf+pos,psz15+49,cbFilename);
			pos+=cbFilename;

			for (int c=0; c<3; c++) {
				buf[pos++]=0x04+c; // 04=SessionID, 05=IP, 06=Port
				pos+=EvaUtil::write16(buf+pos,4); // Length: 4 bytes
				memcpy(szLen,psz15+9+8*c,8);
				szLen[8]=0;
				pos+=EvaUtil::write32(buf+pos,strtol(szLen,NULL,16));
			}

			buf[pos++]=0x07; // Format (A=GIF C=JPG)
			pos+=EvaUtil::write16(buf+pos,1); // Length: 1 byte
			buf[pos++]=psz15[8];

			buf[pos++]=0x08; // File Agent Key
			pos+=EvaUtil::write16(buf+pos,16); // Length: 16 bytes fixed
			memcpy(buf+pos,psz15+33,16);
			pos+=16;
			...
			buf[pos++]=0xff; // Content
			pos+=EvaUtil::write16(buf+pos,len); // Length: 1 byte
			*psz15=0x15;
			memcpy(buf+pos,psz15,len);
			pos+=len;
			pMsg=psz15+len;
		}
	}
...
SendIMReplyPacket::SendIMReplyPacket(qqclient* client, unsigned char *buf, int len)
