...
int EvaRequestFacePacket::putBody(unsigned char *buf)
{
	...
	unsigned short tmp2;
	tmp2 = htons(m_client->data.file_token.len /*tokenLength*/) & 0xffff;
	memcpy(buf+pos, &tmp2, 2);
	pos+=2;
	
	memcpy(buf+pos, m_client->data.file_token.data,m_client->data.file_token.len/* fileAgentToken, tokenLength*/);
	pos+=m_client->data.file_token.len/* tokenLength*/;
...
EvaRequestFaceReplyPacket::EvaRequestFaceReplyPacket(qqclient* client, unsigned char *buf, int len)
	: EvaPicInPacket(client, buf, len, 12)
{
}
