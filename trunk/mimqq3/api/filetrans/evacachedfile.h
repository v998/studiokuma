...
public:
	...
	EvaCachedFile(const string &srcDir, const string &srcFilename, const unsigned int size);
	// for loading, this only source file involved
	EvaCachedFile(const string &srcDir, const string &srcFilename);

	~EvaCachedFile();

	void setDestFileDir(const string &dir);
	void setCheckValues(const unsigned char *fileNameMd5, const unsigned char *fileMd5);
	const bool setFileInfo(const string &fileName, const unsigned int size);
	...
	inline const string &getDir() const { return m_DirPath; }
	inline const string &getFileName() const { return m_FileName; }
private:
	bool m_IsLoading;
	string m_DirPath;
	string m_FileName;
	string m_InfoFileName;
	string m_CachedFileName;
	char m_FileNameMd5[16]; // we received one
	char m_FileMd5[16]; // we received
	unsigned int m_FileSize;
	ErrorState m_State;
	map<unsigned int, unsigned int> m_FragInfo;
	...
	static const bool calculateFileMd5(const string& fullpath, char *md5Buf);
