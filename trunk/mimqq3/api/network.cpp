...
extern "C" void* qqclient_keepalive( void* data );
...
void CNetwork::processPacket(LPCBYTE lpData, const USHORT len)
{
	...
	InPacket* packet=new InPacket(&m_client, (UCHAR*)lpData, len);
	if(!packet->getLength()){
		...
		case QQ_CMD_REQUEST_TOKEN:
			{
				...
				if (*body!=1) {
					util_log(0,"Malformed QQ_CMD_REQUEST_TOKEN response received");
				} else {
					char szTemp[MAX_PATH]="token=";
					char* pszTemp=szTemp+strlen(szTemp);
					unsigned int len=htons(*(unsigned short*)(body+4));
					util_log(0,"QQ_CMD_REQUEST_TOKEN response received, subcmd=%d, type=%d, len=%d",(int)body[0],htons(*(unsigned short*)(body+1)),len);

					for (int c=0; c<len; c++) {
						sprintf(pszTemp,"%02X",(int)body[6+c]);
						pszTemp+=2;
					}
					util_log(0,szTemp);
				}
				/*
				FILE* fp=fopen("C:\\0xae.bin","wb");
				fwrite(packet->getBody(),packet->getLength(),1,fp);
				fclose(fp);
				*/
			}
			break;
...
#endif
				default:
					util_log(0,"Got a non-text msg(%d), can't process it, ignore it",base.getNormalIMType());
				}
			}
			break;
...
void CNetwork::sendOut(OutPacket* out) {
	// WARNING: sendOut ALWAYS releases out!
#if 0
	out->setQQClient(&m_client);
	qqpacket* p = packetmgr_new_send( &m_client, out->getCommand() );
	if( !p ) return;
	bytebuffer *buf = p->buf;
	out->fill(p->buf->data,&p->buf->size);
	p->need_ack=out->needAck();
	packetmgr_put(&m_client,p);
	delete out;
	...
	qqpacket* p = packetmgr_new_send( &m_client, out->getCommand() );
	if (!p) return;
	out->fill(p->buf->data,&p->buf->pos);
	p->need_ack=out->needAck();

	bytebuffer* buf = p->buf;
	int head_len = m_client.network==TCP ? 13 : 11;
	int out_len = PACKET_SIZE;
	uchar* encrypted=(uchar*)mir_alloc(PACKET_SIZE);
	if( !encrypted ) {
		util_log(0,"Error: encrypted not allocated.");
		return;
	}
	qqencrypt( buf->data, buf->pos, m_client.data.session_key, encrypted, &out_len );
	//for tcp
	if( out_len+head_len+1 <= buf->size ){
		memcpy( buf->data+head_len, encrypted, out_len );
		buf->pos = out_len+head_len;
	}else{
		util_log(0,"encrypted data is too large to store in the packet.");
	}
	mir_free(encrypted);
