...
void CClientConnection::connectionError() {
	util_log("%s: Default Connection Error",m_name.c_str());
}
void CClientConnection::connectionEstablished() {
	util_log("%s: Default Connection Established",m_name.c_str());
}
void CClientConnection::connectionClosed() {
	util_log("%s: Default Connection Closed",m_name.c_str());
}
void CClientConnection::waitTimedOut() {
	util_log("%s: Default Wait Timed Out",m_name.c_str());
}
int CClientConnection::dataReceived(NETLIBPACKETRECVER* nlpr) {
	util_log("%s: Default Data Received",m_name.c_str());
	return 0;
}
bool CClientConnection::crashRecovery() {
	util_log("%s: Default Crash Recovery",m_name.c_str());
	return false;
}
...
void CClientConnection::setServer(const bool udp, LPCSTR host, int port) {
	util_log("[CClientConnection] setServer(udp=%d, host=%s, port=%d)",udp,host,port);
	m_host=host;
...
void __cdecl CClientConnection::ThreadProc(void* secondcrash) {
	...
	__try {
		...
		MIRANDASYSTRAYNOTIFY msn={sizeof(msn),"MIMFetion"};
		swprintf(szTemp,TranslateT("Warning: %S Thread caused an unhandled exception. MIMFetion is trying to resume operation.\nIf this doesn't work, please restart Miranda IM."),getName().c_str());
		util_log("[CClientConnection] Unhandled Exception occurred inside ThreadProc");
		msn.tszInfo=szTemp;
		msn.tszInfoTitle=L"MIMFetion";
		...
		if (!crashRecovery() && m_connection) {
			if (!secondcrash) 
				...
				MessageBox(NULL,TranslateT("MIMFetion is forced to disconnect due to chained crash events. Please reconnect or restart Miranda."),NULL,NIIF_ERROR);
...
#endif
		util_log("[CClientConnection] Unhandled Exception occurred inside _disconnect");
		DebugBreak();
	}
}
...
void CClientConnection::_connect() {
	...
	while (redirect) {
		...
		if (m_connection)
			util_log("[%s] Resuming from exception state",getName().c_str());
		else
			m_connection=(HANDLE)CallService(MS_NETLIB_OPENCONNECTION,(WPARAM)g_hNetlibUser,(LPARAM)&nloc);
		...
		if (m_connection/*=(HANDLE)CallService(MS_NETLIB_OPENCONNECTION,(WPARAM)hNetlibUser,(LPARAM)&nloc)*/) {
			...
			util_log("[%s] Connection loop terminated, reconnect=%s",getName().c_str(),m_redirect?"true":"false");
...
void CClientConnection::disconnect() {
	if (m_connection) {
		util_log("[%s] Send disconnect command",getName().c_str());
		m_stopping=true;
		ForkThread((SocketThreadFunc)&CClientConnection::_disconnect,(LPVOID)m_connection);
		m_connection=NULL;
		//util_log("[%s] Wait for connection close",getName().c_str());
		//WaitForSingleObject(m_hEvDisconnect,INFINITE);
		util_log("[%s] Connection closed, exit disconnect thread",getName().c_str());
	}
}
...
void CClientConnection::registerConnection(CONN_TYPES conntype, CClientConnection* connection )
{
	...
	if (m_connections[conntype]!=NULL) {
		util_log("[CClientConnection] Critical: %s instance recreated! Throwing away old instance!",connection->getName().c_str());
#ifdef _DEBUG
		WCHAR szTemp[MAX_PATH];
	...
	util_log("[CClientConnection]: Register %s instance",connection->getName().c_str());
...
{
	util_log("[CClientConnection] Search to unregister %s instance",connection->getName().c_str());
	if (m_csReg.DebugInfo) {
		...
		for (map<CONN_TYPES,CClientConnection*>::iterator iter=m_connections.begin(); iter!=m_connections.end(); iter++)
			if (iter->second==connection) {
				util_log("[CClientConnection]: Unregister %s instance",iter->second->getName().c_str());
				m_connections.erase(iter->first);
				break;
			}
			LeaveCriticalSection(&m_csReg);
	} else
		util_log("[CClientConnection] Warning: unregisterConnection not executed because m_csReg not initialized");
	//util_log("[CClientConnection] End of unregisterConnection");
}
...
void CClientConnection::unregisterAllConnections()
{
	if (m_csReg.DebugInfo) {
		...
		while (m_connections.size()) {
			...
			util_log("[CClientConnection]: Waiting for %s to destroy",iter->second->getName().c_str());
			delete iter->second;
		...
		util_log("[CClientConnection] Warning: unregisterAllConnections not executed because m_csReg not initialized");
}
#endif

void CClientConnection::send(LPCSTR szData, const int len) {
	if (m_connection) 
		if (Netlib_Send(m_connection,(LPCSTR)szData,len,0/*MSG_NODUMP*/)==SOCKET_ERROR) {
			util_log("[CClientConnection] send socket error!");
		}
}
...
void CClientConnection::disbleWriteBuffer() {
	...
	if (setsockopt(CallService(MS_NETLIB_GETSOCKET,(WPARAM)m_connection,0),SOL_SOCKET,SO_SNDBUF,(char*)&val,sizeof(int))) {
		util_log("[CClientConnection] setsockopt returned %d",WSAGetLastError());
		DebugBreak();
	}
}
