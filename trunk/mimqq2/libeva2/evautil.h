...
public:
	...
	static std::string smileyToText(unsigned short smileyCode);     // get text description of the smiley code
	static unsigned char textToSmiley(const std::string &textTag);      // get smiley code of the text description
	...
	static void calcSuns(const unsigned short level, int *suns, int *moons, int *stars);
		...
		char en[16];
		char py[16];
		unsigned char index06;
		unsigned char index09;
	} textMap;
	...
	static int smileyToFileIndex( const unsigned char smileyCode);
private:	
