...
unsigned long Dirs[10] = {0x005FFFFF, 0x005FFFFF, 0x001EFFF7, 0x004000FA,
                         0x004000FA, 0x007FFF9F, 0x00FFFFFF, 0x00FFFFFF,
                         0x00DFFFFF, 0x000000FF};

/* FIXME: SE90PCI: GPIO functions does not have CardData*, so need to add flag here
          However if mixed cards are used, it will cause problems
*/
static bool vt1720 = false;
...
void SetGPIOData(IOPCIDevice *dev, IOMemoryMap *map, unsigned long data)
{
    ...
    if (!vt1720) dev->ioWrite8(CCS_GPIO_DATA2, (data & (0xFF0000)) >> 16, map);
...
void SetGPIOMask(IOPCIDevice *dev, IOMemoryMap *map, unsigned long data)
{
    ...
    if (!vt1720) dev->ioWrite8(CCS_GPIO_MASK2, (data & (0xFF0000)) >> 16, map);
...
unsigned long GetGPIOData(IOPCIDevice *dev, IOMemoryMap *map)
{
    ...
    if (!vt1720) data = (data << 16) | dev->ioRead16(CCS_GPIO_DATA, map);
    return data;
}
...
int card_init(struct CardData *card)
{
    ...
    if ((dev->ioRead8(CCS_I2C_STATUS, card->iobase) & CCS_I2C_EPROM) != 0)
    {
        ...
        switch (subvendor)
        {
            ...
            case VT1724_SUBDEVICE_SE90PCI:
            {
                card->SubType = SE90PCI;
                card->Specific.NumChannels = 2;
                card->Specific.HasSPDIF = true;
                vt1720=true;
                IOLog("Found Onkyo SE90PCI card!\n");
                break;
            }
    ...
    if (card->SubType == REVO71)
       ...
       SetGPIOMask(dev, card->iobase, 0x00BFFF05);
        ...
        SetGPIOMask(dev, card->iobase, 0x000000FF);
    else
