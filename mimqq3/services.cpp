...
#define READINFO_B(x) READ_B(NULL,x,value); itoa(value,szSignature,10); details.push_back(szSignature)
	...
	MIMPROC(TestService) {
		...
		/*
		qqpacket* p = packetmgr_new_send( &m_client, 0xae );
		if( !p ) return 0;
		bytebuffer *buf = p->buf;
		put_byte(buf,0x01);
		put_word(buf,0x07);
		put_int(buf,0x00);
		post_packet( &m_client, p, SESSION_KEY );
		*/
		prot_user_request_token(&m_client,0,0,8,0);
...
int __cdecl CNetwork::SendMsg(HANDLE hContact, int flags, const char* msg) {
	...
	if (uid) {
		...
		if (packet) {
#if 0
			prot_im_send_msg(&m_client,uid,(char*)msg);
			delete packet;
			retseq=1;
#else
			EvaHtmlParser parser;
