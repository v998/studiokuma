...
void CQunImage::doProcessEvent() {
	...
	currentIndex = -1;
	if(!*(LPDWORD)m_network->m_client.data.file_key) {
		util_log(0,"[CQunImage] doProcessEvent -- file key not set!!!, return");
		clearManager();
		return;
	}
...
void CQunImage::sendPacket(EvaPicOutPacket *packet) {
	...
	int len;
	packet->setQQClient(&m_network->m_client);
	packet->fill(buf, &len);

	/*
	FILE* fp=fopen("mimqq3-qi.bin","wb");
	fwrite(buf,len,1,fp);
	fclose(fp);
	DebugBreak();
	*/
...
int CQunImage::dataReceived(NETLIBPACKETRECVER* nlpr) {
	...
	if(nlpr->buffer[0] == FAMILY_05_TAG && nlpr->buffer[nlpr->bytesAvailable-1] == FAMILY_05_TAIL){
		EvaPicInPacket *packet = new EvaPicInPacket(&m_network->m_client, nlpr->buffer+nlpr->bytesUsed, nlpr->bytesAvailable-nlpr->bytesUsed);
		...
		while(pLen>0 && pLen <= nlpr->bytesAvailable-nlpr->bytesUsed){
			...
			delete packet;
			packet = new EvaPicInPacket(&m_network->m_client, nlpr->buffer+nlpr->bytesUsed, nlpr->bytesAvailable-nlpr->bytesUsed);
...
void CQunImage::doRequestPic(CustomizedPic pic) {
	...
	// packet->setFileAgentToken(m_network->m_client.data.file_token.data, m_network->m_client.data.file_token.len);
...
void CQunImage::processRequestAgentReply(const EvaPicInPacket *in) {
	...
	RequestAgentReplyPacket *packet = new RequestAgentReplyPacket(&m_network->m_client, in->getRawBody(), in->getRawBodyLength());
	packet->parse();
...
void CQunImage::processTransferReply(const EvaPicInPacket *in) {
	EvaPicTransferReplyPacket *packet = new EvaPicTransferReplyPacket(&m_network->m_client, in->getRawBody(), in->getRawBodyLength());
	packet->parse();
...
void CQunImage::processRequestStartReply(const EvaPicInPacket *in) {
	EvaRequestStartReplyPacket *packet = new EvaRequestStartReplyPacket(&m_network->m_client, in->getRawBody(), in->getRawBodyLength());
	packet->parse();
...
void CQunImage::doRequestAgent() {
	util_log(0,__FUNCTION__);
	EvaRequestAgentPacket *packet = new EvaRequestAgentPacket(m_network->m_client.data.file_token.data, m_network->m_client.data.file_token.len);
	if (m_hWndPopup) {
