...
std::string EvaHtmlParser::generateSendFormat(qqclient* client, std::string & fileName, const unsigned int agentSessionID, 
			const unsigned int agentIP, const unsigned short agentPort )
{
	...
	*scCh+=scLen;
	...
	char tmp[17];
	//tmp = new char[17];
	memcpy(tmp, client->data.file_key, 16); // Packet::getFileAgentKey()
	tmp[16]=0x00;
