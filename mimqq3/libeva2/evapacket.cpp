...
Packet::Packet(qqclient* client, unsigned char *buf, int *len)
{
	int pos;
...
OutPacket::OutPacket(const short command, const bool needAck) :
	Packet(QQ_VERSION, command, ((startSequenceNum++)%0xffff))
{
                            ...
                            unsigned char *body, int *bodyLen) 
{
	...
	default:{
		...
		qqencrypt(b,length,sessionKey,body,bodyLen);
		}
	}
}
...
InPacket::InPacket(qqclient* client, unsigned char *buf, int len)
	: Packet(client, buf, &len),
	bodyLength(0)
{
...
void InPacket::decryptBody(unsigned char * buf, int len)
{
	...
	switch (this->getCommand()) {
		case QQ_CMD_LOGIN_VERIFY:
			ret=qqdecrypt(buf,len,qqClient->data.verify_key2,decryptedBuf,&bodyLength);
			break;
		case QQ_CMD_LOGIN_GET_INFO:
			ret=qqdecrypt(buf,len,qqClient->data.login_info_key3,decryptedBuf,&bodyLength);
			break;
		case QQ_CMD_LOGIN_SEND_INFO:
		case QQ_CMD_LOGIN_GET_LIST:
			ret=qqdecrypt(buf,len,qqClient->data.login_info_key2,decryptedBuf,&bodyLength);
			break;
		default:
			ret=qqdecrypt(buf,len,qqClient->data.session_key,decryptedBuf,&bodyLength);
	}
	...
	unsigned char * passwordKey = this->qqClient->md5_pass2;
	unsigned char * sessionKey = this->qqClient->data.session_key;
