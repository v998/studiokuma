...
GetOnlineFriendReplyPacket::GetOnlineFriendReplyPacket(qqclient* client, unsigned char *buf, int len) 
...
FriendChangeStatusPacket::FriendChangeStatusPacket(qqclient* client, unsigned char* buf, int len)
	: InPacket(client, buf, len), status(NULL)
{
}
...
int ChangeStatusPacket::putBody(unsigned char *buf) 
{
	...
	*(unsigned int*)(buf+5)=htonl(1); // camera?
...
ChangeStatusReplyPacket::ChangeStatusReplyPacket(qqclient* client, unsigned char *buf, int len) 
	: InPacket(client, buf, len)
{ 
}
