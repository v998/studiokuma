...
{
	// tokenLength = length;
	cryptPosition = 14 + length; //tokenLength;
...
int EvaRequestAgentPacket::putBody(unsigned char *buf)
{
	...
	unsigned short tmp2;
	tmp2 = htons(m_client->data.file_token.len/*tokenLength*/);
	memcpy(buf+pos, &tmp2, 2); pos+=2;
	
	memcpy(buf+pos, m_client->data.file_token.data,m_client->data.file_token.len /*fileAgentToken, tokenLength*/); pos+=m_client->data.file_token.len/* tokenLength*/;
...
RequestAgentReplyPacket::RequestAgentReplyPacket(qqclient* client, unsigned char *buf, int len)
	: EvaPicInPacket(client, buf, len, 8)
{
}
