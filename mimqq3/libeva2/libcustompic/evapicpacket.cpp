...
EvaPicPacket::EvaPicPacket(qqclient* client, unsigned char *buf, int len)
	: key(NULL)
{
	m_client=client;
	parseHeader(buf);
...
EvaPicPacket::EvaPicPacket(qqclient* client, unsigned char *buf, int *len, const int cryptStart)
...
: EvaPicPacket(QQ_CLIENT_0F5F /*QQ_CLIENT_VERSION*/, command, ((sequenceStart++)%0xffff))
{
	mNeedAck = ack;
	resendCount = 5;
}
...
int EvaPicOutPacket::putHead(unsigned char *buf) 
{
	...
	int id = htonl(m_client->number/* getQQ()*/);
	memcpy(buf+pos, &id, 4);
	pos+=4;
...
{
	if(*(LPDWORD)m_client->data.file_key==0 /*fileAgentKey == NULL*/) {     
		fprintf(stderr, "EvaPicOutPacket->encryptBody: file agent key not set yet, set encrypted length to 0\n");
		*bodyLen = 0;
		return;
	} 
	// EvaCrypt::encrypt(b, length, key?key:m_client->data.file_key, body, bodyLen); 
	qqencrypt( b, length, key?key:m_client->data.file_key, body, bodyLen );	
}
...
EvaPicInPacket::EvaPicInPacket(qqclient* client, unsigned char *buf, int len)
	: EvaPicPacket(client, buf, len),decryptedBuf(NULL)
{
...
EvaPicInPacket::EvaPicInPacket(qqclient* client, unsigned char *buf, int len, const int cryptStart)
	: EvaPicPacket(client, buf, &len, cryptStart),
...
void EvaPicInPacket::decryptBody(unsigned char * buf, int len)
{
	...
	if(*(LPDWORD)m_client->data.file_key == 0) {     
	...
	//int ret = EvaCrypt::decrypt(buf + start, len - start, m_client->data.file_key, bodyBuf, &bodyLen); 
	int ret=qqdecrypt(buf+start,len-start,m_client->data.file_key,bodyBuf,&bodyLen);
	if(ret == 0) {
		bodyLength = 0;
