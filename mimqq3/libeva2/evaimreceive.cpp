...
ReceiveIMPacket::ReceiveIMPacket(qqclient* client, unsigned char *buf, const int len)
...
int NormalIMBase::readHeader(const unsigned char *buf)
{
	...
	type = READ16(buf+pos); // content_type in myqq
	pos+=2;

	// myqq process_buddy_im() ended here
...
{
	int pos=0;
	// Next line referenced to "buf->pos += 4; 	//0000001" in myqq process_buddy_im_text()
	...
	if (!memcmp(buf+pos,"MSG",4)) {
		unsigned short sz;
		char* pszTemp;
		unsigned char type;
		std::string msg;
		
		pos+=8; //'M' 'S' 'G' 00 00 00 00 00
		/*sendTime=*/ntohl(*(int*)(buf+pos));
		pos+=4;

		pos+=5; // 15 46 88 03 00 >27 05 01 09 00 86 22

		mHasFontAttribute=true;
		blue=buf[pos++];
		green=buf[pos++];
		red=buf[pos++];
		fontSize=buf[pos++];
		...
		sz=ntohs(*(int*)(buf+pos)); //pass font name
		pos+=2;
		pszTemp=(char*)mir_alloc(sz+1);
		memcpy(pszTemp,buf+pos,sz);
		pos+=sz;
		pszTemp[sz]=0;
		mir_utf8decodecp(pszTemp,936,NULL);
		this->fontName.assign(pszTemp);
		mir_free(pszTemp);

		pos+=2;	//00 00 2009-2-7 Huang Guan, updated

		while (pos<len) {
			type=buf[pos++];
			sz=htons(*(unsigned short*)(buf+pos));
			pos+=2;

			while (pos<len) {
				if (type!=0x01 && buf[pos]==0xff || (type==0x01 && buf[pos]==0x01))
					break;
				else {
					pos++;
					pos+=htons(*(unsigned short*)(buf+pos));
					pos+=2;
				}
			}
			pos++;
			...
			switch (type) {
				...
				case 0x02: //face
					if (sz==2) {
						...
						pszTemp=(char*)mir_alloc(16);
						sprintf(pszTemp,"[face:%d]",*(unsigned char*)(buf+pos+1));
						msg.append(pszTemp);
						mir_free(pszTemp);
					} else {
		...
		pszTemp=mir_strdup(msg.c_str());
		mir_utf8decodecp(pszTemp,936,NULL);
		this->message.assign(ReceiveIMPacket::convertToShow(pszTemp, QQ_IM_NORMAL_REPLY));
		mir_free(pszTemp);
	} else {
		int i=0;
		while( (buf[pos++]) != 0x00){
...
// 			printf("\n");
	...
	externalID = ntohl(*(int*)(buf+pos)); // the external Qun ID
	pos+=4;
	...
	if (source==QQ_RECV_IM_QUN_IM_09) {
		unsigned short sz;
		char* pszTemp;
		unsigned char type;
		std::string msg;
...
{
	int pos=4; // QQ2009 has 4 zero-padding bytes
	qq = READ32(buf+pos); pos+=4; // your buddy qq number
