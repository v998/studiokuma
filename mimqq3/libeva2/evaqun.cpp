...
QunReplyPacket::QunReplyPacket(qqclient* client, unsigned char *buf, int len)
...
#if 0 // This part is completely incompatible with QQ2009
/*
...
int QunReplyPacket::parseRequestAllRealNames(unsigned char *buf, int len)
{
	...
	while(pos<len){
		...
		nameLen = buf[pos++];
		if (nameLen>0) {
			// Meaningless if nameLen==0 (Actually happened!) :)
...
#if 0 // This packet is completely incompatible with QQ2009
/***************************************************************************************************************/
