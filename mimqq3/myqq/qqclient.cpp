...
// NOTE: This function is modified for MirandaQQ3, so it is not in sync with myqq
#define INTERVAL 1
void qqclient_keepalive( qqclient* qq )
{
	...
	if( qq->process == P_LOGGING || qq->process == P_LOGIN || qq->process == P_VERIFYING ){
		packetmgr_check_packet( qq, 2 /*5*/ ); // MIMQQ Modified
		if( qq->process == P_LOGIN ){
			//1次心跳/分钟 
			if( counter % ( 1 *30*INTERVAL) == 0 ){
				prot_user_keep_alive( qq );
			}
#ifdef MIRANDAQQ_EXPORTS
			if (!qq->mimnetwork->IsConservative()) {
#endif
...
int qqclient_login( qqclient* qq )
{
	...
	// qq->packetmgr.recv_threadid=0;
	prot_login_touch( qq );
	// qq->packetmgr.recv_threadid=GetCurrentThreadId();
	//keep
	qq->keep_alive_counter = 0;
#endif
	return 0;
