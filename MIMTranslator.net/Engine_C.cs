...
{
    class Engine_C: Engine_Interface
    {
        ArrayList whiteList;
        ArrayList blackList;
        ArrayList superList;
            ...
            while ((s=sr.ReadLine())!=null)
            {
                ...
                while (isSuper || (pos=MatchList(s,whiteList,pos))>-1)
                {
                    ...
                    if (pos > -1)
                    {
                        ...
                        if (MatchList(s2, blackList, 0) == -1 && pos!=endpos)
                        {
                            al.Add(s2);
                        }
                        pos = endpos + 1;
        ...
        public void ReadConfig(XmlDocument xd, XmlNodeList whiteNodeList, XmlNodeList blackNodeList, XmlNodeList superNodeList)
        {
            ...
            for (int c = 0; c < 3; c++)
            {
                ...
                foreach (XmlNode xn in xd.SelectNodes("/config/wordlist/c/"+s+"/item"))
                {
                    al.Add(xn.InnerText);
                }
            }
