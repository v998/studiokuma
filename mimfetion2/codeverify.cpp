...
CCodeVerifyWindow::CCodeVerifyWindow(HINSTANCE hInstance, LPWSTR pszText, LPSTR pszURL, CLibWebFetion* webqq):
				...
				case IDC_CODEIMAGE:
					{
						...
						sprintf(szPath+strlen(szPath),"verycode-%s.jpg",m_webqq->GetID());

						if (!(pszData=m_webqq->GetHTMLDocument(m_imagepath,m_webqq->GetReferer(CLibWebFetion::WEBQQ_REFERER_PTLOGIN),&dwSize))) {
					...
					break;
				case IDOK:
					GetDlgItemTextA(hwndDlg,IDC_CODE,m_code,5);
				case IDCANCEL:
